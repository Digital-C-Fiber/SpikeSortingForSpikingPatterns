...
columns <- c("F1", "Precision", "Recall", "FNR")
...
for (col_name in columns) {
  ...
  formula_fixed <- as.formula(paste0(col_name, " ~ Model "))
  
  # mit Random-Effekt (1 | Dataset) => gemischtes Modell
  formula_mixed <- as.formula(paste0(col_name, " ~ Model  + (1 | Dataset)"))
  
  # Beta-Formel nutzt shifted_col anstelle von col_name
  formula_beta <- as.formula(paste0(shifted_col, " ~ Model  + (1 | Dataset)"))
  ...
  formula_lmer_logit <- as.formula(paste0(logit_col, " ~ Model  + (1 | Dataset)"))
